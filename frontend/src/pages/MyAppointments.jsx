...
const MyAppointments = () => {
  ...
  const getUserAppointments = async () => {
    if (!token || !userData) {
      console.log('No token or userData, skipping fetch');
      return;
    }
    setLoading(true);
    try {
        ...
        <div>
          {appointments.map((item, index) => (
            <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b" key={index}>
              <div>
                <img className="w-32 bg-indigo-50 rounded-lg" src={item.docData.image} alt={`${item.docData.name}'s profile`} />
              </div>
              ...
              <div className="flex flex-col gap-2 justify-end">
                ...
                {item.isCompleted && (
                  <button className="sm:min-w-48 py-2 border border-green-400 rounded text-green-400">Completed</button>
                )}
              </div>
            </div>
          ))}
